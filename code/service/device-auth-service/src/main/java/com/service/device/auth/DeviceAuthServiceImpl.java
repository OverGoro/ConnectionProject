...
public class DeviceAuthServiceImpl implements DeviceAuthService {
    ...
    public DeviceTokenBlm createDeviceToken(UUID deviceUid) {
        ...
        Date expiresAt =
                Date.from(createdAt.toInstant().plus(deviceTokenDuration));
        UUID tokenUid = UUID.randomUUID();

        // Генерируем токен
        String tokenString = deviceTokenGenerator.generateDeviceToken(deviceUid,
                tokenUid, createdAt, expiresAt);
        DeviceTokenBlm deviceTokenBlm = DeviceTokenBlm.builder()
                .token(tokenString).uid(tokenUid).deviceUid(deviceUid)
    ...
    public DeviceTokenBlm getDeviceToken(UUID deviceUid) {
        ...
        DeviceTokenBlm deviceTokenBlm =
                deviceTokenRepository.findByDeviceUid(deviceUid);
    ...
    public void revokeDeviceToken(UUID deviceUid) {
        ...
        deviceAccessTokenRepository
                .revokeByDeviceTokenUid(deviceTokenBlm.getUid());
    ...
    public DeviceTokenBlm validateDeviceToken(DeviceTokenBlm deviceToken) {
        log.info("Validating device token for device: {}",
    ...
    public DeviceTokenBlm validateDeviceToken(String deviceTokenString) {
        DeviceTokenBlm deviceTokenBlm =
                deviceTokenGenerator.getDeviceTokenBlm(deviceTokenString);
        deviceTokenValidator.validate(deviceTokenBlm);
    ...
    @Override
    @Transactional
    public Pair<DeviceAccessTokenBlm, DeviceTokenBlm> createDeviceAccessToken(
            DeviceTokenBlm deviceToken) {
        log.info("Creating device access token for device token: {}",
                deviceToken.getUid());

        deviceTokenValidator.validate(deviceToken);

        // Проверяем, нет ли активного access token
        if (deviceAccessTokenRepository
                .hasDeviceAccessToken(deviceToken.getUid())) {
            throw new com.connection.device.token.exception.DeviceAccessTokenExistsException(
                    "Active device access token already exists for device token: "
                            + deviceToken.getUid());
        }

        Date createdAt = new Date();
        Date expiresAt = Date
                .from(createdAt.toInstant().plus(deviceAccessTokenDuration));
        UUID accessTokenUid = UUID.randomUUID();

        // Генерируем access token
        String accessTokenString =
                deviceAccessTokenGenerator.generateDeviceAccessToken(
                        deviceToken.getUid(), createdAt, expiresAt);

        DeviceAccessTokenBlm deviceAccessTokenBlm = DeviceAccessTokenBlm
                .builder().token(accessTokenString).uid(accessTokenUid)
                .deviceTokenUid(deviceToken.getUid()).createdAt(createdAt)
        ...
        log.info(
                "Device access token created successfully for device token: {}",
                deviceToken.getUid());
    ...
    public DeviceAccessTokenBlm refreshDeviceAccessToken(
        ...
        String newAccessTokenString =
                deviceAccessTokenGenerator.generateDeviceAccessToken(
                        deviceAccessToken.getDeviceTokenUid(), createdAt,
                        expiresAt);

        DeviceAccessTokenBlm newDeviceAccessTokenBlm = DeviceAccessTokenBlm
                .builder().token(newAccessTokenString).uid(newAccessTokenUid)
    ...
    @Override
    public DeviceAccessTokenBlm validateDeviceAccessToken(
            DeviceAccessTokenBlm deviceAccessToken) {
    ...
    public DeviceAccessTokenBlm validateDeviceAccessToken(
        ...
        DeviceAccessTokenBlm deviceAccessTokenBlm = deviceAccessTokenGenerator
                .getDeviceAccessTokenBlm(deviceAccessTokenString);
        deviceAccessTokenValidator.validate(deviceAccessTokenBlm);
    ...
    public Map<String, Object> getHealthStatus() {
        try {
            ...
            return Map.of("status", "DEGRADED", "service", "device-service",
                    "timestamp", System.currentTimeMillis(), "auth-service",
                    "UNAVAILABLE", "error", e.getMessage());
        }
    }
}
