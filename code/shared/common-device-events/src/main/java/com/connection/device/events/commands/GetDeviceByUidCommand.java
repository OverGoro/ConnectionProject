...
public class GetDeviceByUidCommand extends Command {
    ...
    public GetDeviceByUidCommand() {
        super(DeviceEventConstants.COMMAND_GET_DEVICE_BY_UID, "unknown",
                DeviceEventConstants.DEVICE_COMMANDS_TOPIC);
        this.deviceUid = null;
    }

    /** . */
    public GetDeviceByUidCommand(UUID clientUid, UUID deviceUid,
            String sourceService) {
        super(DeviceEventConstants.COMMAND_GET_DEVICE_BY_UID, sourceService,
                DeviceEventConstants.COMMAND_GET_DEVICE_BY_UID);
