...
public class DeviceRepositorySqlImpl implements DeviceRepository {
    ...
    private static final String SELECT_DEVICE =
            "SELECT uid, client_uuid, device_name, device_description";
    private static final String FROM_DEVICE = " FROM core.device";

    private static final String SELECT_DEVICE_BY_UID =
            SELECT_DEVICE + FROM_DEVICE + " WHERE uid = :uid";
    private static final String SELECT_DEVICES_BY_CLIENT =
            SELECT_DEVICE + FROM_DEVICE + " WHERE client_uuid = :client_uuid";
    private static final String SELECT_DEVICE_BY_CLIENT_AND_NAME = SELECT_DEVICE
            + FROM_DEVICE
            + " WHERE client_uuid = :client_uuid AND device_name = :device_name";
    ...
    private static final String UPDATE_DEVICE =
            "UPDATE core.device SET device_name = "
                    + ":device_name, device_description = :device_description "
                    + "WHERE uid = :uid";

    private static final String DELETE_DEVICE =
            "DELETE FROM core.device WHERE uid = :uid";
    ...
    public void add(DeviceBlm device) throws DeviceAlreadyExistsException {
        ...
        if (exists(device.getUid())) {
            throw new DeviceAlreadyExistsException(
                    "Device with UID " + device.getUid() + " already exists");
        }
    ...
    public void update(DeviceBlm device) throws DeviceNotFoundException {
        ...
        if (!exists(device.getUid())) {
            throw new DeviceNotFoundException(
                    "Device with UID " + device.getUid() + " not found");
        }
    ...
    public DeviceBlm findByUid(UUID uid) throws DeviceNotFoundException {
        ...
        params.addValue("uid", uid);
        try {
            DeviceDalm dalDevice = jdbcTemplate.queryForObject(
                    SELECT_DEVICE_BY_UID, params, deviceRowMapper);
    ...
    public List<DeviceBlm> findByClientUuid(UUID clientUuid) {
        ...
        List<DeviceDalm> dalDevices = jdbcTemplate
                .query(SELECT_DEVICES_BY_CLIENT, params, deviceRowMapper);

        // Конвертация списка Dalm в Blm
        return dalDevices.stream().map(converter::toBlm).toList();
    }
    ...
    public boolean exists(UUID uid) {
        ...
        params.addValue("uid", uid);
        try {
            jdbcTemplate.queryForObject(SELECT_DEVICE_BY_UID, params,
                    deviceRowMapper);
            return true;
        } catch (EmptyResultDataAccessException e) {
            return false;
    ...
    public boolean existsByClientAndName(UUID clientUuid, String deviceName) {
        ...
        try {
            jdbcTemplate.queryForObject(SELECT_DEVICE_BY_CLIENT_AND_NAME,
